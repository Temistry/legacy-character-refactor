...
| `CharacterTuningTable` | `src/refactored/Data/CharacterTuningTable.*` | Character 종류별 tuning 값을 code-level table로 모은다. |
| `SkillDefinitionTable` | `src/refactored/Data/SkillDefinitionTable.*` | skill id, cooldown, cost, power 값을 data object로 조회한다. |
...
| `CharacterTuningTable` | `CharacterTuning`, `FindCharacterTuning` | 타입별 tuning 값을 계산 함수 밖에서 조회한다. |
| `SkillDefinitionTable` | `SkillDefinition`, `FindSkillDefinition`, `ToSkillData` | skill id 기반 값을 `SkillData`로 변환한다. |
